 ...
 function writeNotices() {
  ...
  if (noticesElm) {
   ...
   Object.entries(notices).map(([p, n]) => {
    ...
    }</div><div><p class="p-0 lh-1">${n.noticeLink ? `<a href="${n.noticeLink}">${n.title}</a>` : n.title}</p><p class="p-0 text-muted" style="font-size: 0.75rem;">${n.body || ""}</p><p class="p-0 text-muted" style="font-size: 0.75rem;"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#ccc" class="bi bi-clock-fill" viewBox="0 0 18 18">
