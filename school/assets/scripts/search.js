...
window.onload = async () => {
  ...
  if (postQuery) {
    let post = {};
    await onValue(ref(db, "websites/mums/posts/" + postQuery), (s) => {
      post = s.val();
      if (!post) post = { title: "No post found!" };
      let postsElm = document.getElementById("post");
      if (postsElm) {
        postsElm.innerHTML = postsElm.innerHTML = `<h2 class="card-title mb-2" style="font-family: ${
  ...
  if (noticeQuery) {
    let n = {};
    await onValue(ref(db, "websites/mums/notices/" + noticeQuery), (s) => {
      n = s.val();
